	...
	struct device_attribute *attr, const char *sysfsbuf, size_t count)
{
	...
	if (kstrtoint(sysfsbuf, 10, &val) < 0)
		return -EINVAL;
	...
	struct device_attribute *attr, const char *sysfsbuf, size_t count)
{
	...
	aspeed_lpc_set_ipmi_bt_addr(aspeed_lpc, addr & 0xffff);
	...
	struct device_attribute *attr, const char *sysfsbuf, size_t count)
{
	...
	if (kstrtouint(sysfsbuf, 10, &addr) < 0)
		return -EINVAL;

	aspeed_lpc_set_ipmi_ibt_addr(aspeed_lpc, addr & 0xffff);
